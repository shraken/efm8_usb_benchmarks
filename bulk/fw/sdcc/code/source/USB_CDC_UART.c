...
extern __bit cs_Line_State_Update;			// update line state

//-----------------------------------------------------------------------------
// Global Variables
//-----------------------------------------------------------------------------

volatile TLine_Coding __idata uart_line_coding = {		// line coding structure and its default value
	115200,		// baudrate
	0,			// stop bit:	1
	0,			// parity:		none
	8			// data bits:	8
...
volatile __bit uart_DTR = FALSE;				// Line status output
volatile __bit uart_RTS = FALSE;
...
__bit TXReady;								// TX buffer has data to transmit
__bit RXReady;								// RX buffer has room
...
__bit Set_Line_Coding( TLine_Coding __idata * setting )
{
	BYTE cnt;
	BYTE __idata * src;
	BYTE __idata * dst;
	...
	// Copy setting
	src = (BYTE __idata *)setting;
	dst = (BYTE __idata *)&uart_line_coding;
...
int COMPutMultiByte( BYTE *dt, int len )
{
		BYTE i;
	
		// check if multibyte stream exceeds buffer, if it does
		// then do not copy to buffer and return error
		if ((RXcount + len) >= RXBUFSIZE) {
				return -1;
		}
			
		// copy byte by byte to tail pointer of the ring buffer and
		// if we hit the end of the buffer then roll over
		for (i = 0; i < len; i++) {
				*RXWTPtr = *dt++;
			
				if (RXWTPtr == (RXBuffer + (RXBUFSIZE - 1))) {
						RXWTPtr = RXBuffer;
				} else {
						RXWTPtr++;
				}
		...
		/*
		// copy the multibyte stream into buffer
		memcpy( RXWTPtr, dt, len );
		RXWTPtr += len;
		RXcount += len;
		
		// set RX ready flag to TRUE if the buffer is not full
		RXReady = (RXcount != RXBUFSIZE);
		*/
