...
extern __bit   setup_handled;				// flag that indicates setup stage is handled or not
...
extern __bit Ep_Status1;					// Contains status bytes for EP 0-2
extern __bit Ep_StatusOUT2;
extern __bit Ep_StatusIN2;
...
void swap_endian_long( ULONG __idata *lptr );
...
void CS_Set_Line_Coding_Complete(void)
{
	...
	swap_endian_long( (ULONG __idata *)cs_temp_buffer );

#endif // end of BIG_ENDIAN

	Set_Line_Coding( (TLine_Coding __idata *)cs_temp_buffer );
}
...
void CS_Get_Line_Coding(void)
{
		...
		&& (Setup.wLength.i == sizeof(TLine_Coding)) )
	{
		...
		src = (BYTE __idata *)&uart_line_coding;
...
BYTE __code serialStateResponse[ CS_SERIAL_STATE_NUM ] = 
{
...
*/
	BYTE tmp[4];
	BYTE __idata * ptr = (BYTE __idata *)lptr;
