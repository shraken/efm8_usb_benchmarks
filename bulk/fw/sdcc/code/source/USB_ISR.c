...
__bit   setup_handled;					// flag that indicates setup stage is handled or not
UINT  DataSize;							// Size of data to return
BYTE* DataPtr;							// Pointer to data to return
__bit   send_eq_requested;				// flag that indicates that the data to send on TX
										// equals to the requested by Setup wLength

// Holds the status for each endpoint
BYTE Ep_Status0   = EP_IDLE;
__bit Ep_Status1    = EP_HALT;
__bit Ep_StatusIN2  = EP_HALT;
__bit Ep_StatusOUT2 = EP_HALT;
...
//
//-----------------------------------------------------------------------------
void Usb_ISR(void) __interrupt 8			// Top-level USB ISR
{
